...
async function getCommitFiles(kit: OctoKit): Promise<File[]> {
  const responses: ReposGetCommitResponse[] = await kit.paginate(kit.rest.repos.getCommit.endpoint.merge({
...
export async function getChangedFiles(token: string) {
  ...
  const files = pr?.number ? await getPullRequestFiles(kit) : await getCommitFiles(kit);
  return files.filter(({ status }) => status !== "removed").map(({ filename }) => filename);
}
